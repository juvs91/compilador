...
def p_factor_1(p):
    '''Factor1 : MINUS Seen_Unary_Operator Constant Seen_Operand Gen_Quad0
               ...
               | MINUS Seen_Unary_Operator Length Gen_Quad0
               | PLUS MINUS Seen_Unary_Operator Length Gen_Quad0
...
def p_seen_param_call(p):
    ...
    if ("[]" in type and "[]" in sem.func_table[state.current_call][1][state.param_counter]):
        ...
        if (sem.var_table[state.current_call][arg][1] < 0):  # If variable is not yet assigned
            dir = max(map(lambda x: x[1][1] + x[1][2][0], sem.var_table[state.current_call].items()) + [
                0])  # Gets the last available address
            sem.var_table[state.current_call][arg][1] = dir  # Updates the starting address
            sem.var_table[state.current_call][arg][2] = [var[1][2][0] / max(sum(var[1][2][1:state.arr_current_dim + 1]),
                                                                            1)] + var[1][2][
                                                                                  state.arr_current_dim + 1:]  # Updates the size and dimensions of the variable with the passed parameter
            sem.var_table[state.current_call][arg][4] = var[1][4][
                                                        state.arr_current_dim:]  # Updates the m of each dimension
...
mem_dict = dict(
    map(swap, sem.var_table[sem.global_str].items()) + map(swap_str, sem.var_table[sem.constant_str].items()))
